...
def graph_to_tome_space(data, distribution_vec):
    ...
    weighted_avg_tome = np.zeros(data.shape[1])

    for i in range(len(distribution_vec)):
        weighted_avg_tome += data[i,:] * distribution_vec[i]

    return data[imax,:], weighted_avg_tome
